...
from grpclib.events import DispatchServerEvents
...
async def test_invalid_method(loop):
    ...
    headers = [(':method', 'GET')]
    await request_handler({}, stream, headers, ProtoCodec(),
                          DispatchServerEvents(), release_stream)
...
async def test_deadline(loop):
    ...
    task = loop.create_task(
        request_handler(methods, stream, headers, ProtoCodec(),
                        DispatchServerEvents(), release_stream)
    )
